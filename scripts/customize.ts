...
async function getCustomizationAnswers(): Promise<CustomizationAnswers> {
  console.log("🎨 Welcome to Deno 2 Starter Customization!");
  console.log("Let's set up your project with some basic information.\n");

  const siteName = await prompt("📝 What is your site/app name? (e.g., 'My Awesome App'):");
  
  const siteDescription = await prompt("📄 Describe your app in one sentence:");

  const colorOptions = Object.keys(colors);
  const selectedColor = await select("🎨 Choose your primary brand color:", colorOptions);
 ...
 * 
 * ⚠️  CUSTOMIZED: This file was generated by the customization script
 * ✏️  To make further changes, edit this file directly or run: deno run scripts/customize.ts
 */
