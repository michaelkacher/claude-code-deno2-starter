...
export const handler = define.handlers({
  async GET(ctx) {
    ...
    console.log(
      ...
      currentUser ? `${currentUser.email} (${currentUser.role})` : "null"
    );
    ...
    const [usersResult, statsResult] = await Promise.all([
      ...
      handleApiFetch<AdminUsersData["stats"]>(`${baseUrl}/api/admin/stats`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      }),
...
export default define.page<AdminUsersData>(function AdminUsersPage({ data }) {
  if (data.error) {
    return (
