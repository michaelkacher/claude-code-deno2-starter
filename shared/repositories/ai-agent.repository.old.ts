...
export interface AgentPerformanceHistory {
  ...
  success: boolean;
  errorMessage?: string;
  qualityScore?: number; // 0-100 based on human review
}
...
export class AIAgentRepository extends BaseRepository<AIAgent> {
  ...
  async updateStatus(id: string, newStatus: AIAgent['status']): Promise<AIAgent | null> {
    ...
    await this.update(id, updatedAgent);
    return updatedAgent;
  }
  ...
  async assignTask(agentId: string, taskId: string): Promise<AIAgent | null> {
    ...
    await this.update(agentId, updatedAgent);
    return updatedAgent;
  }
  ...
  async updatePerformanceMetrics(agentId: string, qualityScore: number): Promise<AIAgent | null> {
    const agent = await this.findById(agentId);
    if (!agent) return null;

    // This would typically involve more complex performance tracking
  ...
  async scheduleMaintenanceMode(agentId: string, duration: number = 3600000): Promise<AIAgent | null> {
    ...
    // Schedule return to active status (in real implementation, this would use a job queue)
    setTimeout(async () => {
